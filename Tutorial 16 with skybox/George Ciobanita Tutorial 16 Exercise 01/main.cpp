...
//Tutorial 16 Test
SkyDomeShaderClass* g_pSkyDomeShader = 0;
SkyDomeClass* g_pSkyDome = 0;
ID3D11RasterizerState* m_rasterState;
ID3D11RasterizerState* m_rasterStateNoCulling;
ID3D11DepthStencilState* m_depthStencilState=0;
ID3D11DepthStencilState* m_depthDisabledStencilState=0;
...
void ShutdownD3D()
{
	...
	if (g_pSkyDomeShader)
	{
		g_pSkyDomeShader->Shutdown();
		delete g_pSkyDomeShader;
		g_pSkyDomeShader = 0;
	}
	if (g_pSkyDome)
	{
...
HRESULT InitialiseGraphics() //03-01
{
	...
	g_pSkyDomeShader = new SkyDomeShaderClass;
	if (!g_pSkyDomeShader)
	{
		return false;
	}
	...
	bool result;
	result = g_pSkyDomeShader->Initialize(g_pD3DDevice, g_hWnd);
	if (!result)
	{
		return false;
	}
	...
	result = g_pSkyDome->Initialize(g_pD3DDevice);
	if (!result)
	{
		MessageBox(g_hWnd, "Could not initialize the sky dome object.", "Error", MB_OK);
		return false;
	}
	...
	result = g_pD3DDevice->CreateRasterizerState(&rasterDesc, &m_rasterState);
	if (FAILED(result))
	{
		return false;
	}
	...
	result = g_pD3DDevice->CreateRasterizerState(&rasterDesc, &m_rasterStateNoCulling);
	if (FAILED(result))
	{
		return false;
	}
	...
	result = g_pD3DDevice->CreateDepthStencilState(&depthDisabledStencilDesc, &m_depthDisabledStencilState);
	if (FAILED(result))
	{
		return false;
	}
...
void RenderFrame(void)
{
	...
	g_pImmediateContext->ClearDepthStencilView(g_pZBuffer, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 10000.0f, 0);
	...
	XMFLOAT3 cameraPos = XMFLOAT3(g_pCamera->GetX(), g_pCamera->GetY(), g_pCamera->GetZ());
	TurnOffCulling();
	TurnZBufferOff();
	XMMATRIX skydomeWorldMatrix;
	skydomeWorldMatrix = XMMatrixTranslation(g_pCamera->GetX(), g_pCamera->GetY(), g_pCamera->GetZ());

	g_pSkyDome->Render(g_pImmediateContext);
	g_pSkyDomeShader->Render(g_pImmediateContext, g_pSkyDome->GetIndexCount(), skydomeWorldMatrix,view, projection, g_pSkyDome->GetApexColor(), g_pSkyDome->GetCenterColor());
...
void TurnOffCulling()
{
	...
	g_pImmediateContext->RSSetState(m_rasterStateNoCulling);
