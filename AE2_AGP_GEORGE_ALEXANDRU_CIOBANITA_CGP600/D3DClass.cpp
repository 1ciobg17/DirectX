...
bool D3DClass::Initialize(int screenWidth, int screenHeight, bool vsync, HWND hwnd, bool fullscreen, float screenDepth, float screenNear)
{
	...
	//Create a diretx graphics interface factory
	...
	//When a match is foud store the numerator and the denominator of the refresh rate for that monitor
	...
	//set the scen line ordering and scaling to unspecified
	...
	//Create the render targt view with the back buffer pointer
	...
	//Initialize the world matrix to the identity atrix
	D3DXMatrixIdentity(&m_worldMatrix);

	//Create an orthographic projection matrix for 2D rendering
	D3DXMatrixOrthoLH(&m_orthoMatrix, (float)screenWidth, (float)screenHeight, screenNear, screenDepth);

	//clear the state using the device
	ZeroMemory(&blendStateDescription, sizeof(D3D11_BLEND_DESC));

	//create an alpha enabled blend state descripotion
...
{
	//Before shutting down set to windowed mode or when you release the swap chain it will throw an expection
	if (m_swapChain)
	{
...
void D3DClass::BeginScene(float red, float green, float blue, float alpha)
{
	...
	//setup the folor to clear the buffer to
	color[0] = red;
	color[1] = green;
	color[2] = blue;
	color[3] = alpha;
...
void D3DClass::GetProjectionMatrix(D3DMATRIX& projectionMatrix)
{
	projectionMatrix = m_projectionMatrix;
	return;
}

void D3DClass::GetWorldMatrix(D3DMATRIX& worldMatrix)
{
	worldMatrix = m_worldMatrix;
	return;
}

void D3DClass::GetOrthoMatrix(D3DMATRIX& orthoMatrix)
{
