...
void ObjFileModel::SetVertexBuffer(void)
{
	UINT stride = sizeof(MODEL_POS_TEX_NORM_VERTEX);
	UINT offset = 0;
	pImmediateContext->IASetVertexBuffers(0, 1, &pVertexBuffer, &stride, &offset);
...
int ObjFileModel::loadfile(char* fname)
{
	...
	if (pFile==NULL) { "Failed to open model file";fname; return 0 ;}
	...
	if (fbuffer == NULL) {fclose(pFile); "Failed allocate memory for model file";fname; return 0 ;}

	// copy file into memory
	actualsize = fread(fbuffer,1,fbuffersize,pFile); // actualsize may be less than fbuffersize in text mode as \r are stripped
	if (actualsize == 0) {fclose(pFile); "Failed to read model file";fname; return 0 ;}
...
{
	tokenptr=0; // token pointer points to v1 element of buffer
	...
	{	
		...
		if(!getnexttoken(tokenstart, tokenlength)) continue; // get v1 token on line, go to next line if v1 token is \n
		...
		if(strncmp(&fbuffer[tokenstart], "v ", 2)==0) // VERTEX POSITION - note the space in the string is needed (see vt, etc)
		{
			...
			if(!success) {char s[100] = "ERROR: Badly formatted vertex, line : "; _itoa(line, &s[strlen(s)], 10); strcat(s, " : "); strcat(s, filename.c_str());  s; }
		...
		else if(strncmp(&fbuffer[tokenstart], "vt", 2)==0) // TEXTURE COORDINATES
		{
			...
			if(!success) {char s[100] = "ERROR: Badly formatted texture coordinate, line : "; _itoa(line, &s[strlen(s)], 10); strcat(s, " : "); strcat(s, filename.c_str());  s; }
		...
		else if(strncmp(&fbuffer[tokenstart], "vn", 2)==0)  // NORMALS
		{
			...
			if(!success) {char s[100] = "ERROR: Badly formatted normal, line : "; _itoa(line, &s[strlen(s)], 10); strcat(s, " : "); strcat(s, filename.c_str()); s; }
		...
		else if(strncmp(&fbuffer[tokenstart], "f ", 2)==0)  // FACE - only deals with triangles so far
		{
			...
			if(!success) {char s[100] = "ERROR: Badly formatted face, line : "; _itoa(line, &s[strlen(s)], 10); strcat(s, " : "); strcat(s, filename.c_str());  s; }
		}
