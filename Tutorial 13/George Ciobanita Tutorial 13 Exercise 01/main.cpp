...
HRESULT InitialiseGraphics() //03-01
{
	...
	g_pModel->SetZPos(20.0f);
	g_pModel->SetXPos(5.0f);
	g_pModel->SetScale(0.5f);
	g_pModel->SetYRot(90.0f);

	g_pModel_2 = new Model(g_pD3DDevice, g_pImmediateContext);
	g_pModel_2->LoadObjModel("assets/sphere.obj");

	g_pModel_2->SetZPos(20);
	g_pModel_2->SetXPos(-20);
	g_pModel_2->SetScale(2.0f);
...
void RenderFrame(void)
{
	...
	if (IsKeyPressed(DIK_UP)) g_pCamera->Forward(1.0f);

	if (IsKeyPressed(DIK_DOWN)) g_pCamera->Forward(-1.0f);

	if (IsKeyPressed(DIK_LEFT)) g_pCamera->Rotate(-1.0f);

	if (IsKeyPressed(DIK_RIGHT)) g_pCamera->Rotate(1.0f);
	...
	g_pModel_2->LookAt_XZ(g_pModel->GetXPos(), g_pModel->GetZPos());

	g_pModel_2->Draw(&view, &projection);
