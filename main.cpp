...
char g_TutorialName[100] = "Tutorial 19 Exercise 01\0";
...
ID3D11Buffer* g_pSkyVertexBuffer;
ID3D11VertexShader* g_pSkyVertexShader;
ID3D11PixelShader* g_pSkyPixelShader;
ID3D11ShaderResourceView* g_pSkyTexture0;
ID3D11InputLayout* g_pSkyInputLayout;
...
void ShutdownD3D()
{
	...
	if (g_cameraModel)
	{
		delete g_cameraModel;
		g_cameraModel = NULL;
	}
	if (g_direct_input) g_direct_input->Release();
	if (g_pRasterSolid != 0)
	{
		delete g_pRasterSolid;
		g_pRasterSolid = NULL;
	}
	if (g_pRasterSkyBox!= 0)
	{
		delete g_pRasterSkyBox;
		g_pRasterSkyBox = NULL;
	}
	if (g_pDepthWriteSolid != 0)
	{
		delete g_pDepthWriteSolid;
		g_pDepthWriteSolid = NULL;
	}
	if (g_pDepthWriteSkyBox != 0)
	{
		delete g_pDepthWriteSkyBox;
		g_pDepthWriteSkyBox = NULL;
	}
	if (g_pSkyTexture0) g_pSkyTexture0->Release();
	if (g_pSkyInputLayout) g_pSkyInputLayout->Release(); //03-01
}
...
HRESULT InitialiseGraphics() //03-01
{
	...
	g_cameraModel = new Model(g_pD3DDevice, g_pImmediateContext);
	g_cameraModel->LoadObjModel("assets/cube.obj");
	...
	g_cameraNode->SetModel(g_cameraModel);
	...
	hr = g_pD3DDevice->CreateRasterizerState(&rasterizerDesc, &g_pRasterSolid);

	rasterizerDesc.CullMode = D3D11_CULL_FRONT;
	hr = g_pD3DDevice->CreateRasterizerState(&rasterizerDesc, &g_pRasterSkyBox); 
	...
	hr = g_pD3DDevice->CreateDepthStencilState(&depthDesc, &g_pDepthWriteSolid);

	depthDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ZERO;

	hr = g_pD3DDevice->CreateDepthStencilState(&depthDesc, &g_pDepthWriteSkyBox);

	hr = g_pD3DDevice->CreateBuffer(&bufferDesc, NULL, &g_pSkyVertexBuffer); //create the buffer
	...
	//Copy the vertices into the buffer
	D3D11_MAPPED_SUBRESOURCE skyMS;

	//Lock the buffer to allow writing
	g_pImmediateContext->Map(g_pSkyVertexBuffer, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &skyMS);

	//copy the data
	memcpy(skyMS.pData, vertices, sizeof(vertices));

	//unlock the buffer
	g_pImmediateContext->Unmap(g_pSkyVertexBuffer, NULL);

	ID3DBlob *skyVS, *skyPS;
	hr = D3DX11CompileFromFile("sky_shader.hlsl", 0, 0, "VShader", "vs_4_0", 0, 0, 0, &skyVS, &error, 0);
	...
	hr = D3DX11CompileFromFile("sky_shader.hlsl", 0, 0, "PShader", "ps_4_0", 0, 0, 0, &skyPS, &error, 0);
	...
	//create shader objects
	hr = g_pD3DDevice->CreateVertexShader(skyVS->GetBufferPointer(), skyVS->GetBufferSize(), NULL, &g_pSkyVertexShader);
	...
	// PS -> skyPS ******************************
	hr = g_pD3DDevice->CreatePixelShader(skyPS->GetBufferPointer(), skyPS->GetBufferSize(), NULL, &g_pSkyPixelShader);
	...
	//set the shader objects as active
	g_pImmediateContext->VSSetShader(g_pSkyVertexShader, 0, 0);
	g_pImmediateContext->PSSetShader(g_pSkyPixelShader, 0, 0);

	g_pD3DDevice->CreateInputLayout(iedesc, ARRAYSIZE(iedesc), skyVS->GetBufferPointer(), skyVS->GetBufferSize(), &g_pSkyInputLayout);
	...
	D3DX11CreateShaderResourceViewFromFile(g_pD3DDevice, "assets/skybox02.dds", NULL, NULL, &g_pSkyTexture0, NULL); 

	g_pImmediateContext->IASetInputLayout(g_pSkyInputLayout);
...
void RenderFrame(void)
{
	...
	// if/def is so we disable drawing or changing anything else to debug
	// the skybox - get it on screen!
#if 1
	...
	if (IsKeyPressed(DIK_W))
	{
		...
		g_cameraNode->SetXPos(g_pCamera->GetX());
		g_cameraNode->SetYPos(g_pCamera->GetY());
		g_cameraNode->SetZPos(g_pCamera->GetZ());
		XMMATRIX identity = XMMatrixIdentity();
		// update tree to reflect new camera position
		g_root_node->UpdateCollisionTree(&identity, 1.0);
		if (g_cameraNode->CheckCollision(g_root_node) == true)
		{
			// if there is a collision, restore camera and camera node positions
			g_pCamera->Forward(-.1);
			g_cameraNode->SetXPos(g_pCamera->GetX()); //15
			g_cameraNode->SetYPos(g_pCamera->GetY());//15
			g_cameraNode->SetZPos(g_pCamera->GetZ());//15
		}
	}
...
void DrawSkyBox()
{
	...
	XMMATRIX projection, world=XMMatrixIdentity(), view;
	world = XMMatrixTranslation(g_pCamera->GetX(), g_pCamera->GetY(), g_pCamera->GetZ());
	...
	//add textures and anti aliasing
	g_pImmediateContext->PSSetSamplers(0, 1, &g_pSampler0);
	g_pImmediateContext->PSSetShaderResources(0, 1, &g_pSkyTexture0);
